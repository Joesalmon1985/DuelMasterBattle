...
static func _seed_prereqs(adv: Node, area: Dictionary) -> void:
	...
	# For now, just ensure the test mode flag is set
	adv.state["village_test_profile"] = selected_profile
	adv.state["village_test_area_id"] = area["id"]
	
	# Seed pilot story flags
	for story in _Profiles.get_pilot_stories(selected_profile):
		adv.state["story_" + story["id"] + "_active"] = true
		adv.state["story_" + story["id"] + "_beat"] = 0
