...
`PersonState`: id, name, alive/status, affiliation, cultural appearance, role, job/workplace, home/current area/position, preferences (3 typed traits initially), goal IDs, relationship map, known facts, dialogue profile, history refs. Anchor role is a flag/profile on the same person, not a duplicate NPC. One active worker per building job by default. A job change preserves identity and creates a new replacement next accounting tick. No ageing at era transition.
