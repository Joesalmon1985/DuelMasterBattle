...
|------|---------|------------|
| `player_wizard.png` | Player wizard portrait | Created (not yet on board) |
| `enemy_wizard.png` | Enemy wizard portrait | Created (not yet on board) |
| `duel_background.png` | Panel/background frame | Created (not yet on board) |

## Icons (`godot_project/assets/icons/`)

| File | Purpose | Integrated |
|------|---------|------------|
| `magic_*.png` (×10) | Magic type picker buttons | Yes — `magic_picker.gd` |
| `point_*.png` (×4) | Shield/Body/Staff/Mind | Created (labels in slots) |
| `feedback_hit.png` | Hit marker | Partial — feedback text labels |
...
## Verification
Godot headless: `tools/run_godot_tests.sh` includes `test_assets.gd`.
