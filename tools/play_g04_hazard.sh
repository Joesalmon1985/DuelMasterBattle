...
export DMB_FIXTURE="FX-HAZARD"
export DMB_SEED="408"
export DMB_SAVE_SLOT="g04_hazard"
RESOLUTION="${DMB_RESOLUTION:-450x800}"
if [[ "${1:-}" == "--resolution" ]]; then RESOLUTION="${2:?}"; fi
echo "G04 FX-HAZARD seed=408 save=g04_hazard resolution=$RESOLUTION"
exec "$GODOT_BIN" --path "$ROOT/godot_project" --resolution "$RESOLUTION" \
  res://client/scenes/g04_hazard_shell.tscn
