...
def _sprite_for_role(role: str) -> str:
    r = role.lower()
    # Restrict generated fixtures to sprite keys already used by the existing
    # E36B production-overworld fixture, rather than inventing new asset names.
    if any(x in r for x in ("healer", "physician", "herbal")):
        return "healer"
    if any(x in r for x in ("woodcutter", "forester", "logger", "woodsman")):
        return "woodcutter"
    if any(x in r for x in ("miner", "ore", "quarry", "smith", "blacksmith", "miller", "mill")):
        return "worker_m"
    if any(x in r for x in ("priestess", "woman", "wife", "daughter", "mother")):
        return "villager_f"
    return "villager_m"
